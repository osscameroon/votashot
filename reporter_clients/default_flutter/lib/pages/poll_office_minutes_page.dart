...
class PollOfficeMinutesPage extends StatefulWidget {
  const PollOfficeMinutesPage({super.key});

  @override
  State<PollOfficeMinutesPage> createState() => _PollOfficeMinutesPageState();
}

class _PollOfficeMinutesPageState extends State<PollOfficeMinutesPage> {
  final _formKey = GlobalKey<FormBuilderState>();
  bool _submitting = false;
  static const String _statKey = 'poll_office_minutes_sent';
  int _sentCount = 0;
    ...
    try {
      ...
      for (final item in images) {
        ...
        var result = await uploader.putToS3(
          ...
          bucket: creds.bucket,
          objectKey: name,
          keyPrefix: '${creds.prefix}/minutes',
          virtualHost: true,
          bytes: bytes,
          contentType: ct,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(context.l10n.minutesTitle),
